...
export const latestInsights = asyncHandler(async (req: Request, res: Response) => {
  const rawLimit = Array.isArray(req.query.limit) ? req.query.limit[0] : req.query.limit;
  const limit = rawLimit ? Number(rawLimit) : 3;

  return sendData(res, await getLatestAIInsights(getUserId(req), Number.isFinite(limit) ? limit : 3));
});

export const expenseAdvice = asyncHandler(async (req: Request, res: Response) => {
...
export const groceryAdvice = asyncHandler(async (req: Request, res: Response) => {
...
export const workLimitWarning = asyncHandler(async (req: Request, res: Response) => {
