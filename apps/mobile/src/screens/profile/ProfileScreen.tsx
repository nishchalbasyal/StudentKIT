...
function statusLabel(status?: StudentStatus) {
  return (
    statusOptions.find((item) => item.value === status)?.label ?? "Student"
  );
}
...
export function ProfileScreen() {
  ...
  const [studentStatus, setStudentStatus] =
    useState<StudentStatus>("INTERNATIONAL");
  ...
  if (!isAuthenticated) {
    return (
      <AppScreen title="Profile" subtitle="Your account summary.">
        <EmptyState
          title="Guest mode"
          message="Sign in or create an account to unlock your profile, sync, and cloud backups."
        />
              ...
              try {
                if (!(await canReachApi())) {
                  Alert.alert(
                    ...
                    "Connect to the internet and make sure the Student Kit server is running before logging in or registering.",
                  );
                  return;
                }
  ...
  const save = async () => {
    ...
    if (
      ...
      return Alert.alert(
        "Check wage",
        "Default hourly wage must be a positive number.",
      );
    }
    if (
      parsedLimit !== undefined &&
      (!Number.isFinite(parsedLimit) || parsedLimit < 1 || parsedLimit > 366)
    ) {
      return Alert.alert(
        "Check work limit",
        "Yearly work limit must be between 1 and 366 days.",
      );
    }
    ...
    <AppScreen title="Profile" subtitle="Your account summary.">
      <View style={styles.headerCard}>
        <Pressable
          ...
          accessibilityLabel={isAuthenticated ? "Edit profile" : "Log in"}
          onPress={() => {
            if (!isAuthenticated) {
              navigation.navigate("Auth", { screen: "Login" });
              return;
        ...
        <Text style={styles.meta}>
          {countryLabel(user.country)} - {user.currency}
        </Text>
        <View style={styles.statusPill}>
          <Text style={styles.statusText}>
            {statusLabel(user.studentStatus)}
          </Text>
        </View>
        <AppButton
      ...
      <View style={styles.card}>
        ...
        <InfoRow
          label="Work country"
          value={countryLabel(workSettings?.workCountry ?? user.country)}
        />
        <InfoRow
          label="Yearly work limit"
          value={`${workSettings?.yearlyWorkLimitDays ?? 140} days`}
        />
        <InfoRow
          label="Default wage"
          value={`${formatCurrency(Number(workSettings?.defaultHourlyWage ?? user.hourlyWageDefault ?? 14.05), user.currency)}/h`}
        />
        ...
        <View style={styles.statsGrid}>
          <Stat
            ...
            value={formatCurrency(
              ...
              summary?.currency ?? user.currency,
            )}
            tone="green"
          />
          <Stat
            label="Work streak"
            value={`${summary?.workStreak ?? 0} days`}
            tone="blue"
          />
          <Stat
            label="Expenses"
            value={`${summary?.expensesTracked ?? 0}`}
            tone="red"
          />
          <Stat
            label="Tasks done"
          ...
          <Stat
            label="Split groups"
            value={`${summary?.activeSplitGroups ?? 0}`}
            tone="green"
          />
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        <ActionRow
          ...
          onPress={() =>
            Alert.alert(
              "Linked accounts",
              "No linked accounts yet. Email login is active for this account.",
            )
          }
        />
        <ActionRow
          ...
          onPress={() =>
            Alert.alert(
              "Export my data",
              "Your export includes profile, settings, tasks, expenses, work entries, and split data.",
            )
          }
        />
        <ActionRow
          ...
          onPress={() =>
            Alert.alert(
              "Privacy",
              "Your data is scoped to your account. Split groups are visible only to group members.",
            )
          }
        />
        <ActionRow
